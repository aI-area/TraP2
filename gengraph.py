...
def preprocess_input_graph(G, labels, normalize_adj=False):
    ...
    if normalize_adj:
        sqrt_deg = np.diag(1.0 / np.sqrt(np.sum(adj, axis=0,
                                                dtype=float).squeeze()))
...
def gen_syn1(nb_shapes=80, width_basis=300, feature_generator=None, m=5):
    ...
    Args:
        nb_shapes         :
        The number of shapes (here 'houses') that should be
        added to the base graph.
        width_basis       :
        The width of the basis graph (here 'Barabasi-Albert' random graph).
        feature_generator :
        A `FeatureGenerator` for node features. If `None`,
        add constant features to nodes.
        m                 :
        number of edges to attach to existing node (for BA graph)

    Returns:
        G                 :  A networkx graph
        role_id           :  A list with length equal to number
        of nodes in the entire graph (basis + shapes). role_id[i] is the ID
        of the role of node i. It is the label.
...
def gen_syn2(nb_shapes=100, width_basis=350):
    ...
    Start with Barabasi-Albert graph and add node features indicative of
    a community label.

    Args:
        nb_shapes         :  The number of shapes (here 'houses')
        that should be added to the base graph.
        width_basis       :  The width of the basis graph
        (here 'Barabasi-Albert' random graph).

    Returns:
        G                 :  A networkx graph
        label             :  Label of the nodes
        (determined by role_id and community)
        name              :  A graph identifier
    """
    basis_type = "ba"
    ...
    # Create two grids
    mu_1, sigma_1 = np.array([-1.0] * 2 + random_mu), \
                    np.array([0.5] * 2 + random_sigma)
    mu_2, sigma_2 = np.array([1.0] * 2 + random_mu), \
                    np.array([0.5] * 2 + random_sigma)
...
def gen_syn3(nb_shapes=80, width_basis=300, feature_generator=None, m=5):
    ...
    Args:
        nb_shapes         :  The number of shapes (here 'grid')
        that should be added to the base graph.
        width_basis       :  The width of the basis graph
        (here 'Barabasi-Albert' random graph).
        feature_generator :  A `FeatureGenerator` for node features.
        If `None`, add constant features to nodes.
        m                 :  number of edges to attach to
        existing node (for BA graph)
