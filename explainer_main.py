...
def arg_parse():
    ...
    parser.add_argument("--clean-log", action="store_true",
                        help="If true, cleans the specified "
                             "log directory before running.")
    ...
    parser.add_argument(
        "--epochs", dest="num_epochs", type=int,
    ...
    parser.add_argument(
        "--explain-node", dest="explain_node",
        type=int, help="Node to explain."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        help="whether to run Explainer on multiple "
             "Graphs from the Classification task for "
    ...
    parser.add_argument(
        ...
        type=int,
        help="whether to run Explainer on multiple "
             "nodes from the Classification task for "
             "examples in the same class.",
    )
    parser.add_argument(
        "--align-steps",
    ...
    parser.add_argument(
        "--method", dest="method", type=str, help="Method. Possible values: "
                                                  "base, att, grad."
    )
    parser.add_argument(
        "--name-suffix", dest="name_suffix",
...
def match_parameters(args):
    if args.dataset in ["syn1", "syn2"]:
        args.lr = 0.1
        args.num_epochs = 100   # 100
    elif args.dataset in ["syn4"]:
        args.lr = 0.2
        args.num_epochs = 110
    elif args.dataset in ["syn5"]:
        args.lr = 0.02
        args.num_epochs = 150
    return args
...
def main():
    ...
    prog_args = arg_parse()
    prog_args = match_parameters(prog_args)
    ...
    if prog_args.writer:
        ...
        if os.path.isdir(path) and prog_args.clean_log:
           ...
           if not input("Are you sure you want to remove this directory? "
                        "(y/n): ").lower().strip()[:1] == "y": sys.exit(1)
           shutil.rmtree(path)
    ...
    if prog_args.explain_node is not None:
        ...
        if prog_args.multinode_class >= 0:
            ...
            # if nothing is pecified, "rangge(400, 700, 5)" - 60 nodes - are
            # selected
            if prog_args.method == "base":
                method = "exp"
            else:
                method = prog_args.method

            # Mutagenicity,
            if prog_args.dataset in ["syn1"]:
                explainer.explain_nodes_gnn_stats(
                    range(400, 700, 5), prog_args, model=method
                    )
            elif prog_args.dataset in ["syn2"]:
                explainer.explain_nodes_gnn_stats(
                    range(400, 700, 5), prog_args, model=method
                )
            elif prog_args.dataset in ["syn3"]:
                explainer.explain_nodes_gnn_stats(
                    range(300, 840, 9), prog_args, model=method
                )
            elif prog_args.dataset in ["syn4"]:
                explainer.explain_nodes_gnn_stats(
                    range(511, 871, 6), prog_args, model=method
                )
            elif prog_args.dataset in ["syn5"]:
                explainer.explain_nodes_gnn_stats(
                    range(512, 962, 9), prog_args, model=method
                )
