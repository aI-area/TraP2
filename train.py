...
def train(
    ...
    plt.figure()
    plt.plot(train_epochs, math_utils.exp_moving_avg(train_accs, 0.85),
             "-", lw=1)
    if test_dataset is not None:
        plt.plot(best_val_epochs, best_val_accs, "bo",
                 test_epochs, test_accs, "go")
...
def evaluate_node(ypred, labels, train_idx, test_idx):
    ...
    result_test = {
        "prec": metrics.precision_score(labels_test, pred_test,
                                        average="macro"),
        "recall": metrics.recall_score(labels_test, pred_test,
                                       average="macro"),
...
def syn_task1(args, writer=None):
    ...
    G, labels, name = gengraph.gen_syn1(
        feature_generator=featgen.ConstFeatureGen(np.ones(args.input_dim,
                                                          dtype=float))
    )
    num_classes = max(labels) + 1
...
def pkl_task(args, feat=None):
    ...
    if feat is None:
        featgen_const = featgen.ConstFeatureGen(np.ones(args.input_dim,
                                                        dtype=float))
        for G in graphs:
...
def benchmark_task_val(args, writer=None, feat="node-label"):
    ...
    for i in range(10):
        ...
        assign_input_dim = cross_val.prepare_val_data(
...
def arg_parse():
    ...
    softpool_parser.add_argument(
        "--num-pool", dest="num_pool", type=int,
    ...
    parser.add_argument(
        ...
        help="Maximum number of nodes "
             "(ignore graghs with nodes exceeding the number.",
    )
    ...
    parser.add_argument(
        "--input-dim", dest="input_dim", type=int,
    ...
    parser.add_argument(
        "--num-classes", dest="num_classes", type=int,
