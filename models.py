...
class GraphConv(nn.Module):
    def __init__(
        ...
        if not gpu:
            ...
            if att:
                self.att_weight \
                    = nn.Parameter(torch.FloatTensor(input_dim, input_dim))
        else:
            self.weight \
...
class GcnEncoderGraph(nn.Module):
    def __init__(
        ...
        if concat:
            self.pred_input_dim = hidden_dim * (num_layers - 1) \
                                  + embedding_dim
        else:
            self.pred_input_dim = embedding_dim
        self.pred_model = self.build_pred_layers(
            self.pred_input_dim, pred_hidden_dims, label_dim,
    ...
    def construct_mask(self, max_nodes, batch_num_nodes):
        """ For each num_nodes in batch_num_nodes, the first num_nodes
        entries of the corresponding column are 1's, and
        the rest are 0's (to be masked out).
    ...
    def activations_second_hook(self, grad):
        self.second_conv_grads = grad

    def activations_first_hook(self, grad):
        self.first_conv_grads = grad

    def activations_third_hook(self, grad):
        self.third_conv_grads = grad
    ...
    def gcn_forward(
        ...
        Returns:
            ...
            [batch_size x num_nodes x embedding]
            The embedding dim is self.pred_input_dim
        """
        with torch.enable_grad():
            self.first_act, adj_att = conv_first(x, adj)
            self.first_act.register_hook(self.activations_first_hook)
            x = self.first_act
            x = self.act(x)
            if self.bn:
                x = self.apply_bn(x)
        ...
        for i in range(len(conv_block)):
            with torch.enable_grad():
                self.second_act, _ = conv_block[i](x, adj)
                self.second_act.register_hook(self.activations_second_hook)
            x = self.act(self.second_act)
            if self.bn:
                x = self.apply_bn(x)
            x_all.append(x)
            adj_att_all.append(adj_att)
        with torch.enable_grad():
            self.last_act, adj_att = conv_last(x, adj)
            self.last_act.register_hook(self.activations_third_hook)

        x_all.append(self.last_act)
        adj_att_all.append(adj_att)
...
class GcnEncoderNode(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes=None, **kwargs):
        ...
        self.embedding_tensor, adj_att = self.gcn_forward(
            x, adj, self.conv_first, self.conv_block,
            self.conv_last, embedding_mask
        )
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(
        ...
        for i in range(num_pooling):
            ...
            self.assign_conv_first, self.assign_conv_block, \
            self.assign_conv_last = self.build_conv_layers(
                assign_input_dim,
                assign_hidden_dim,
                assign_dim,
                assign_num_layers,
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        for i in range(self.num_pooling):
            ...
            adj = torch.transpose(self.assign_tensor, 1, 2) \
                  @ adj @ self.assign_tensor
            x_a = x
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        """ 
        Args:
            batch_num_nodes: numpy array of number of nodes
            in each graph in the minibatch.
        """
        eps = 1e-7
        loss = super(SoftPoolingGcnEncoder, self).loss(pred, label)
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            pred_adj0 = self.assign_tensor \
            ...
            self.link_loss = -adj * torch.log(pred_adj + eps) \
                             - (1 - adj) * torch.log(1 - pred_adj + eps)
